...
Переход к микросервисной архитектуре будет оправдан при росте нагрузки, появлении нескольких моделей, разных SLA и необходимости независимого масштабирования компонентов.[web:591][web:578]
...
- retraining jobs;
- логирования и доставки событий в очередь.[web:578]
...
- **NGINX** работает как reverse proxy, распределяет запросы, обрабатывает TLS, статику и балансировку нагрузки.[web:576][web:591]

## Бизнес-метрики

Помимо технических метрик используются бизнес-метрики:
- **Expected loss reduction** — ожидаемое снижение потерь от дефолтов;
- **Approval rate at fixed risk** — доля одобренных заявок при фиксированном уровне риска.[web:324]
...
```text
<добавишь после публикации образа>
```